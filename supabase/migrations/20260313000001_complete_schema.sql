-- ══════════════════════════════════════════════════════════════════════════════
-- SUPERPOWER HUB — COMPLETE SCHEMA
-- 20260313000001_complete_schema.sql
--
-- Canonical, idempotent migration for the full data model.
-- Safe to run on a fresh instance or against one with prior incremental
-- migrations already applied.
--
-- Sections:
--   0.  Extensions
--   1.  Shared utility trigger function
--   2.  CRM config — marketplace_categories
--   3.  CRM config — marketplace_locations
--   4.  Ideas library (idea_categories + ideas)
--   5.  Business plans
--   6.  Builder profiles  (/s/[slug] public pages from the /build wizard)
--   7.  Marketplace profiles
--   8.  Marketplace services  (structured name + price, child of profile)
--   9.  Reviews
...
--
-- Replaces the hardcoded MARKETPLACE_CATEGORY_META constant in
-- src/lib/marketplace-data.ts. The /crm/categories page reads and writes here.
--
-- Fields:
--   key        — URL-safe slug used as a foreign key target, e.g. "Creative"
--   name       — Display label shown in the UI
--   emoji      — Single emoji used on cards and badges
--   color      — Primary hex colour for badges, borders, charts
--   wash       — Light tint used for card backgrounds / badge fills
--   sort_order — Ascending integer; controls display order in filters + forms
--   active     — False hides the category from the public UI
...
-- Writes reserved for service-role key (CRM API routes).


-- ── 3. Marketplace locations (CRM-managed) ────────────────────────────────────
--
-- Replaces the hardcoded CAPE_TOWN_TOWNSHIPS array in marketplace-data.ts.
-- Used in the listing creation wizard and the location filter on /marketplace.
--
-- Fields:
--   name       — Township display name, used as a string value in profile.locations[]
--   sort_order — Controls display order in pill filters and dropdowns
--   active     — False removes the township from all public-facing dropdowns
...
-- ── 4. Ideas library ──────────────────────────────────────────────────────────
--
-- 50 zero-rand business ideas, currently hardcoded in src/lib/ideas.ts.
-- Persisting them here lets the CRM add/edit ideas without a code deploy.
--
-- idea_categories — the 5 top-level groupings (business, community, learn,
--                   creative, tech) with bilingual labels and colour tokens.
-- ideas           — individual idea cards with bilingual name + description.

create table if not exists public.idea_categories (
  id         text    primary key,        -- e.g. "business", "creative"
  label      text    not null,           -- English label
  label_sa   text    not null,           -- Afrikaans label
  emoji      text    not null default '⚡',
  color      text    not null default '#22A06B',
  wash       text    not null default '#EDF8F2',
  sort_order integer not null default 0
...
create table if not exists public.ideas (
  id             text    primary key,    -- slug, e.g. "sneaker-rescue"
  category_id    text    not null references public.idea_categories(id),
  emoji          text    not null default '⚡',
  name           text    not null,       -- English name
  name_sa        text    not null,       -- Afrikaans name
  description    text    not null,       -- English description
  description_sa text    not null,       -- Afrikaans description
...
-- ── 5. Business plans ─────────────────────────────────────────────────────────
--
-- Private business planning document generated by the /build wizard.
-- A user can have many plans (multiple ideas, multiple iterations).
-- All section fields default to '' so the record can be created early in the
-- wizard and updated incrementally as each step is completed.
--
-- Fields map directly to the wizard steps and BusinessPlanInput type in the app:
--   idea_id / idea_name     — which idea was chosen (null = custom idea)
--   business_name           — the name the founder chose for their business
--   founder_name            — the entrepreneur's own name
--   neighborhood            — township / wijk selected in step 3
--   problem                 — problem statement (step 4)
--   solution                — solution description (step 5)
--   target_customers        — customer segments (step 6)
--   marketing_and_sales     — marketing strategy (step 7)
--   startup_costs           — cost breakdown, free-form text or JSON (step 8)
--   pricing_and_revenue     — revenue model (step 9)
--   mvp                     — minimum viable product description (step 10)
...
-- ── 6. Builder profiles ───────────────────────────────────────────────────────
--
-- The public-facing "About me" page produced by the /build wizard.
-- Served at /s/[slug]. Distinct from both business_plans (private doc) and
-- marketplace_profiles (service listing) — this is the entrepreneur's profile.
--
-- Fields mirror the ProfileData interface in src/app/build/page.tsx:
--   slug            — unique URL slug, e.g. "zara-designs-langa"
--   name            — founder display name
--   wijk            — township / neighbourhood
--   bio             — AI-generated or manually edited bio paragraph
--   tagline         — one-liner shown under the name
--   story           — "what makes me different" paragraph (wizard step 6)
--   availability    — e.g. "Mon–Fri 8am–5pm" (wizard step 7)
--   promise         — service guarantee / promise to customers (wizard step 8)
--   photo_url       — profile photo (uploaded to storage/builder/{user_id}/)
--   services        — JSONB array of {name, price, description} objects
--   plan            — TEXT[] of 3 action-plan steps generated by the wizard
--   lang            — language used when the profile was created ("en" | "sa")
--   is_published    — false = draft, not publicly accessible
...
-- ── 7. Marketplace profiles ───────────────────────────────────────────────────
--
-- One public listing per authenticated user. The storefront on /marketplace.
--
-- Schema decisions:
--   locations TEXT[]  — multi-area support; values must exist in
--                       marketplace_locations.name (enforced in app, not DB,
--                       for flexibility when locations are renamed)
--   category  TEXT FK — references marketplace_categories.key, cascades on
--                       key rename so listings stay consistent
--   services          — moved to child table marketplace_services (see §8)
--   is_published      — lets a user save a draft before going live
...
--
-- Child rows of marketplace_profiles. Each row is one service offered by the
-- business. Replaces the old free-text services TEXT column.
--
-- Mirrors the ServiceItem interface in src/app/marketplace/new/page.tsx:
--   { name: string; price: string }
--
-- Fields:
--   name        — Service name, e.g. "Logo design"
--   price       — Rand amount as plain text, e.g. "350" (no currency symbol)
--   description — Optional detail shown on the expanded profile
...
-- ── 9. Reviews ────────────────────────────────────────────────────────────────
--
-- One review per (reviewer_id, profile_id) pair — enforced by unique constraint.
-- A user cannot review their own listing — enforced by insert policy.
--
-- Fields:
--   reviewer_name — display name on the review card; can differ from auth name
--   rating        — integer 1–5 stars; displayed as x/10 (avg * 2) in the UI
--   title         — short headline, optional
--   body          — full review text, optional
...
-- ── 10. Storage ───────────────────────────────────────────────────────────────
--
-- Two public buckets, both scoped by user ID so users can only touch their
-- own files. Folder structure: /{user_id}/{filename}
--
-- marketplace — profile photo + service portfolio images
-- builder     — profile photo from the /build wizard
...
-- Marketplace categories (mirrors MARKETPLACE_CATEGORY_META in marketplace-data.ts)
...
-- Marketplace locations (mirrors CAPE_TOWN_TOWNSHIPS in marketplace-data.ts)
...
-- Idea categories (mirrors CATEGORY_META in src/lib/ideas.ts)
...
-- 50 zero-rand business ideas (mirrors IDEAS array in src/lib/ideas.ts)
